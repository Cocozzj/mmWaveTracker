...
iperf_connect(struct iperf_test *test)
{
	...
	// Create the control channel using an ephemeral port
        ...
        // define a max connection timeout such that the following call returns early instead of waiting
        // for 128 seconds for example if not specified, the original version used -1 as not to use
        // any maximum limit for the connection timeout
...
iperf_run_client(struct iperf_test * test)
{
    int startup;
    int result = 0;
    i_errno = IENONE;    // cleared before each run
