...
#include "../../src/main/cpp/test_run.c"
...
void
iperf_set_test_interval(struct iperf_test *ipt, int interval_ms)
{
    // call reporter and stat interval functions, convert ms to seconds
    iperf_set_test_reporter_interval(ipt, (double)interval_ms/1000.0);
    iperf_set_test_stats_interval(ipt, (double)interval_ms/1000);
}
...
void
iperf_set_test_protocol(struct iperf_test *ipt, char proto)
{
    if (proto == 'u') {
        set_protocol(ipt, Pudp); // set protocol to UDP
        iperf_set_test_blksize(ipt, 0); // automatically detect BLKSIZE
    }
}
...
void
iperf_set_test_udp_bandwidth(struct iperf_test *ipt, char * bandwidth)
{
    iperf_set_test_rate(ipt, (uint64_t) unit_atof_rate(bandwidth));
}
...
iperf_json_finish(struct iperf_test *test)
{
    ...
    // print the iperf output in Java using JNI calls instead of in C
    //fprintf(test->outfile, "%s\n", test->json_output_string);
    recv_iperf_data_from_C(test->json_output_string);
    iflush(test);
...
iperf_printf(struct iperf_test *test, const char* format, ...)
{
	...
	if (test->title)
        ...
        char finalLine[1024];
        sprintf(finalLine, "%s:  %s", test->title, linebuffer);
        recv_iperf_data_from_C(finalLine);
    }
    else if (test->role == 's') {
	char linebuffer[1024];
	va_start(argp, format);
